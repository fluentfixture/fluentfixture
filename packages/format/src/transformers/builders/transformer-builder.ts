...
const isIdentityToken = (token: Token): boolean => TypeUtils.isString(token.body) && token.body.length === 0;
...
const getTransformersForDynamicToken = (token : Token, factory: TransformerFactory): Transformer => {
  const transformers = [
    ...
    getFallbackTransformer(TypeUtils.isAssigned(token.fallback) ? token.fallback : undefined),
